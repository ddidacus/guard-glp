...
def run(
    ...
    if do_sample:
        print("[+] Using sampling from LLM...")
    else:
        print("[+] Using greedy decoding from LLM...")
    ...
    train_benign_prompts = [
        sample["prompt"] for sample in train_ds if not sample["adversarial"]
    ]
    ...
    if steering_type == "sv":
        ...
        print(
            f"[GPU {gpu_id}] Computing steering vectors from train split activations..."
        )
        ...
        sv_single = (benign_acts.mean(dim=0) - malicious_acts.mean(dim=0)).squeeze(
            0
        )  # (D,)
        del benign_acts, malicious_acts
        tokenizer.padding_side = "left"
    ...
    for alpha in alpha_list:
        ...
        if steering_type == "sv":
            ...
            glp_generate_fn = script_steer.generate_with_intervention_wrapper(
                seed=42, generate_kwargs=GENERATION_KWARGS
            )
        ...
        def generate_responses(
            ...
            with torch.no_grad():
                output_ids = llm.generate(
                    **inputs, pad_token_id=tokenizer.pad_token_id, **GENERATION_KWARGS
                )
            return [
