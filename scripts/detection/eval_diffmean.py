...
def main(
    ...
    # prefix to hook); diff-of-means operates on the raw LLM activations, not a GLP.
...
def aggregate(out_dir: str) -> dict[str, Any]:
    ...
    good_eval_acts = torch.cat(
        [c["good_eval_acts"] for c in chunks], dim=0
    )  # test benign
    metric_bad_acts = torch.cat(
        [c["metric_bad_acts"] for c in chunks], dim=0
    ...
    print(
        f"  cal_benign: {n_good}  |  test_benign: {n_good_eval}  |  "
    ...
    steering_directions: dict[int, NDArray] = {}
    ...
    torch.save(sv_pt, out_path / "steering_vector.pt")
    print(f"Steering vector saved to {out_path / 'steering_vector.pt'}")
    ...
    for li, layer in enumerate(layers):
        ...
        print(
            f"  Layer {layer:2d}  cal_good={gc.mean():.3f}±{gc.std():.3f}  "
    ...
    results["aggregate"]["mean"] = _agg_section(
        "mean diffmean-score",
        good_eval_scores.mean(1),
        bad_scores.mean(1),
        cal_g_sc=good_cal_scores.mean(1),
        cal_b_sc=metric_bad_scores.mean(1),
    )

    print("\n--- max across layers (most adversarial layer) ---")
    results["aggregate"]["max"] = _agg_section(
        "max diffmean-score",
        good_eval_scores.max(1),
        bad_scores.max(1),
        cal_g_sc=good_cal_scores.max(1),
        cal_b_sc=metric_bad_scores.max(1),
    )
    ...
    m_best = _agg_section(
        ...
        good_eval_scores[:, best_li],
        bad_scores[:, best_li],
        cal_g_sc=good_cal_scores[:, best_li],
        cal_b_sc=metric_bad_scores[:, best_li],
    )
