...
# Symbols re-used by sibling detection scripts (eval_linear_probe, eval_diffmean, ...).
__all__ = [
    "_chunk",
...
def _threshold_metrics(
    labels: NDArray, scores: NDArray, threshold: float
) -> dict[str, Any]:
...
def _find_best_f1_threshold(labels: NDArray, scores: NDArray) -> float:
...
def _classification_metrics(
    ...
    score_name: str,
    verbose: bool = True,
    target_tprs: tuple[Any, ...] = tuple(np.arange(0.60, 1.00, 0.05).round(2)),
    ...
    tpr_results: dict[str, dict[str, Any]] = {}
...
def _make_plots(
    out_dir: Path, named_scores: list[tuple[str, NDArray, NDArray]]
) -> None:
    ...
    colors = cast(Any, plt.cm).tab10(np.linspace(0, 1, max(len(named_scores), 1)))

    # ── PR curve ──────────────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(6, 5))
    for (name, labels, scores), color in zip(named_scores, colors, strict=False):
        precision_arr, recall_arr, _ = precision_recall_curve(labels, scores)
        auprc = average_precision_score(labels, scores)
        ax.plot(
            recall_arr, precision_arr, label=f"{name}  (AP={auprc:.3f})", color=color
        )
    ...
    fig, axes = plt.subplots(
        1, len(named_scores), figsize=(5 * len(named_scores), 4), squeeze=False
    )
    for idx, ((name, labels, scores), _color) in enumerate(
        zip(named_scores, colors, strict=False)
    ):
        ax = axes[0, idx]
        precision_arr, recall_arr, thresholds = precision_recall_curve(labels, scores)
    ...
    print(
        f"  Plots saved: {out_dir / 'pr_curves.png'}  |  {out_dir / 'tpr_fnr_threshold.png'}"
    )
...
def extract_activations(
    ...
    activations = save_acts(
        ...
        text=texts,
        tracedict_config=cast("dict[str, Any]", diffusion_model.tracedict_config),
        token_idx=cast('Literal["last", "mean", "all"]', token_pooling),
...
def extract_log_probs(
    ...
    if precomputed_activations is not None:
        ...
        assert texts is not None, "texts required when precomputed_activations is None"
    ...
    layer_expected_sigma: list[torch.Tensor] = []  # DTE only; None otherwise
...
def extract_reconstruction_errors(
    ...
    with torch.no_grad():
        if precomputed_activations is not None:
            activations = precomputed_activations.to(
                device=device, dtype=torch.bfloat16
            )
        else:
            assert texts is not None, (
                "texts required when precomputed_activations is None"
            )
            activations = save_acts(
                hf_model=llm_model,
                hf_tokenizer=llm_tokenizer,
                text=texts,
                tracedict_config=cast(
                    "dict[str, Any]", diffusion_model.tracedict_config
                ),
                token_idx="last",
                batch_size=batch_size,
            ).to(device=device, dtype=torch.bfloat16)  # (N, L, D)

        errors_per_layer: list[torch.Tensor] = []
        for li, actual_layer in enumerate(layers):
            layer_acts = activations[:, li : li + 1, :]  # (N, 1, D)
            normalized = diffusion_model.normalizer.normalize(
                layer_acts, layer_idx=actual_layer
            )
            ...
            reconstructed = diffusion_model.normalizer.denormalize(
                reconstructed, layer_idx=actual_layer
            )
            err = torch.norm(
...
def main(
    ...
    # knn ref samples are generated by GLP (approx. fineweb, more logically aligned for classification here)
        ...
        print(
            f"[+] Building DTE reference set by sampling kNN-DTE "
    ...
    print(
        f"======= Computing scores for calibration - benign set (GPU {gpu_id}) ======="
    )
    cal_good_results: list[Any] = []
    for i in tqdm(
        range(0, len(cal_good_acts), batch_size),
        desc="cal_good",
        mininterval=30,
        ncols=120,
    ):
    ...
    print(
        f"======= Computing scores for calibration - malicious set (GPU {gpu_id}) ======="
    )
    cal_bad_results: list[Any] = []
    for i in tqdm(
        range(0, len(cal_bad_acts), batch_size),
        desc="cal_bad",
        mininterval=30,
        ncols=120,
    ):
    ...
    for i in tqdm(
        range(0, len(test_good_acts), batch_size),
        desc="test_good",
        mininterval=30,
        ncols=120,
    ):
    ...
    for i in tqdm(
        range(0, len(test_bad_acts), batch_size),
        desc="test_bad",
        mininterval=30,
        ncols=120,
    ):
...
def aggregate(out_dir: str) -> dict[str, Any]:
    ...
    files = sorted(out_path.glob("logprob_results_*.th"))
    assert files, f"No logprob_results_*.th files found in {out_path}"

    chunks = [torch.load(f, map_location="cpu", weights_only=False) for f in files]
    print(f"Loaded {len(chunks)} GPU result file(s) from {out_path}")
    ...
    tensor_keys = (
        ...
        "test_bad_probs",
        "good_expected_sigma",
        "good_eval_expected_sigma",
        "bad_expected_sigma",
        "cal_bad_logp",
        "cal_bad_probs",
        "metric_bad_expected_sigma",
        "good_recon_errors",
        "good_eval_recon_errors",
        "bad_recon_errors",
        "metric_bad_recon_errors",
    )
    cat = {
        k: torch.cat([c[k] for c in chunks], dim=0)
        for k in tensor_keys
        if chunks[0].get(k) is not None
    }

    # These tensors are guaranteed present in their respective method branches
    # (log-prob keys for non-recon, recon keys for recon, sigma keys for DTE); the
    # casts reflect that runtime invariant for the type checker.
    cal_good_logp = cast(torch.Tensor, cat.get("cal_good_logp"))
    cal_bad_logp = cast(torch.Tensor, cat.get("cal_bad_logp"))
    test_good_logp = cast(torch.Tensor, cat.get("test_good_logp"))
    test_bad_logp = cast(torch.Tensor, cat.get("test_bad_logp"))
    # sigmas
    good_es = cast(torch.Tensor, cat.get("good_expected_sigma"))
    good_eval_es = cat.get("good_eval_expected_sigma", good_es)
    bad_es = cast(torch.Tensor, cat.get("bad_expected_sigma"))
    metric_bad_es = cast(torch.Tensor, cat.get("metric_bad_expected_sigma"))
    ...
    good_recon_errors = cast(torch.Tensor, cat.get("good_recon_errors"))
    good_eval_recon_errors = cast(torch.Tensor, cat.get("good_eval_recon_errors"))
    bad_recon_errors = cast(torch.Tensor, cat.get("bad_recon_errors"))
    metric_bad_recon_errors = cast(torch.Tensor, cat.get("metric_bad_recon_errors"))
    ...
    print(
        f"  good_cal: {n_good_cal}  |  good_eval: {n_good_eval}  |  bad: {n_bad}  |  "
    ...
    results: dict[str, Any] = {
        "config": cfg,
        "n_good_cal": n_good_cal,
        "n_good_eval": n_good_eval,
        "n_bad": n_bad,
    ...
    def _agg_section(
        ...
        g_scores: NDArray,
        b_scores: NDArray,
        cal_g_scores: NDArray | None = None,
        cal_b_scores: NDArray | None = None,
        add_to_plot: bool = True,
    ) -> dict[str, Any]:
        if cal_g_scores is None or cal_b_scores is None:
        ...
        # positive class = adversarial (bad=1, good=0)
        cal_lbl = np.concatenate(
            [np.zeros(len(cal_g_scores)), np.ones(len(cal_b_scores))]
        )
        ...
        m = _classification_metrics(
            eval_lbl, scores, label_str, best_f1_threshold=youden_thr
        )
    ...
    if is_recon:
        for li, layer in enumerate(layers):
            ...
            youden_thr = _find_best_f1_threshold(
                cal_labels, np.concatenate([gc_err, mb_err])
            )
            m = _classification_metrics(
                main_labels,
                np.concatenate([g_err, b_err]),
                f"layer {layer} recon_error",
        ...
        for li, layer in enumerate(layers):
            ...
            if has_dte:
                ...
                print(
                    f"          exp_sigma   good={g_es.mean():.4f}±{g_es.std():.4f}  bad={b_es.mean():.4f}±{b_es.std():.4f}"
                )
                # higher expected_sigma = more anomalous = adversarial (positive class)
                youden_thr_es = _find_best_f1_threshold(
                    cal_labels, np.concatenate([gc_es, mb_es])
                )
                m_es = _classification_metrics(
                    main_labels,
                    np.concatenate([g_es, b_es]),
                    f"layer {layer} exp_sigma",
                    best_f1_threshold=youden_thr_es,
                )
                m_es["good_mean"] = float(g_es.mean())
                m_es["bad_mean"] = float(b_es.mean())
                results["per_layer"][key]["expected_sigma"] = m_es
                layer_auprcs.setdefault("expected_sigma", []).append(
                    (layer, m_es["auprc"])
                )
    ...
    if is_recon:
        ...
        results["aggregate"]["mean"]["recon_error"] = _agg_section(
            "mean recon_error",
            good_eval_recon_errors.mean(1).numpy(),
            bad_recon_errors.mean(1).numpy(),
        ...
        results["aggregate"]["max"]["recon_error"] = _agg_section(
            "max recon_error",
            good_eval_recon_errors.max(1).values.numpy(),
            bad_recon_errors.max(1).values.numpy(),
        ...
        for score_key, auroc_list in layer_auprcs.items():
            ...
            print(
                f"  best layer for {score_key}: layer {best_layer} (AUPRC={best_auprc:.4f})"
            )
            m = _agg_section(
                f"best-layer {score_key} (L{best_layer})",
                good_eval_recon_errors[:, best_li].numpy(),
                bad_recon_errors[:, best_li].numpy(),
        ...
        results["aggregate"]["mean"][main_score_key] = _agg_section(
            ...
            -test_good_scores.mean(1).numpy(),
            -test_bad_scores.mean(1).numpy(),
            cal_g_scores=-cal_good_scores.mean(1).numpy(),
            cal_b_scores=-cal_bad_scores.mean(1).numpy(),
        )
        if has_dte:
            # higher expected_sigma = more anomalous = adversarial (positive class)
            results["aggregate"]["mean"]["expected_sigma"] = _agg_section(
                "mean exp_sigma",
                good_eval_es.mean(1).numpy(),
                bad_es.mean(1).numpy(),
                cal_g_scores=good_es.mean(1).numpy(),
                cal_b_scores=metric_bad_es.mean(1).numpy(),
            )
        ...
        results["aggregate"]["max"][main_score_key] = _agg_section(
            ...
            (-test_good_scores).max(1).values.numpy(),
            (-test_bad_scores).max(1).values.numpy(),
            cal_g_scores=(-cal_good_scores).max(1).values.numpy(),
            cal_b_scores=(-cal_bad_scores).max(1).values.numpy(),
        )
        if has_dte:
            # highest expected_sigma across layers
            results["aggregate"]["max"]["expected_sigma"] = _agg_section(
                "max exp_sigma",
                good_eval_es.max(1).values.numpy(),
                bad_es.max(1).values.numpy(),
                cal_g_scores=good_es.max(1).values.numpy(),
                cal_b_scores=metric_bad_es.max(1).values.numpy(),
            )
