...
for gpu_id in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    python scripts/detection/evaluate_classifier.py run --config="$CONFIG" --gpu_id="$gpu_id" &
    PID_LIST+=" $!"
    sleep 5
done
...
python scripts/detection/evaluate_classifier.py aggregate --out_dir="$(python -c "import yaml; print(yaml.safe_load(open('$CONFIG'))['out_dir'])")"
