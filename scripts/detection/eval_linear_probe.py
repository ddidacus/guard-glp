...
__all__ = ["_score_probe", "_train_probe", "aggregate", "main"]
...
def _score_probe(
    probe: LinearProbe, acts: torch.Tensor, device: str, batch_size: int
) -> NDArray:
...
def main(
    ...
    def _extract(texts: list[str], tag: str) -> tuple[torch.Tensor, list[int]]:
        ...
        lens = [
            len(ids)
            for ids in llm_tokenizer(texts, truncation=True, max_length=2048)[
                "input_ids"
            ]
        ]
...
def _run_sanity_checks(
    ...
    device: str,
    good_eval_lengths: NDArray | None,
    bad_lengths: NDArray | None,
    metric_bad_lengths: NDArray | None,
    good_eval_scores: NDArray | None = None,
    bad_scores: NDArray | None = None,
) -> dict[str, Any]:
    ...
    norm_aurocs: dict[str, dict[str, float]] = {}
    ...
    for run_i in range(N_SHUFFLES):
        ...
        all_layer_aurocs: list[float] = []
        for li, _layer in enumerate(layers):
            train_acts_li = torch.cat(
                [good_acts[:, li, :], metric_bad_acts[:, li, :]], dim=0
            )
            probe = _train_probe(
                train_acts_li,
                shuffled,
            ...
            g_sc = _score_probe(
                probe, good_eval_acts[:, li, :], device, probe_batch_size
            )
            b_sc = _score_probe(probe, bad_acts[:, li, :], device, probe_batch_size)
            all_layer_aurocs.append(
                float(roc_auc_score(main_labels, np.concatenate([g_sc, b_sc])))
            )
    ...
    if good_eval_lengths is not None and bad_lengths is not None:
        ...
        length_auroc = float(
            roc_auc_score(main_labels, lr_clf.predict_proba(all_lengths)[:, 1])
        )
        print(
        ...
        buckets = {
            ...
            f"mid ({int(q33) + 1}–{int(q66)} tok)": (combined_lengths > q33)
            & (combined_lengths <= q66),
        ...
        combined_scores = (
            np.concatenate([good_eval_scores.mean(1), bad_scores.mean(1)])
        ...
        for name, mask in buckets.items():
            ...
            if n_good_in < 2 or n_bad_in < 2:
                print(
                    f"  {name}: skipped (too few samples: good={n_good_in} bad={n_bad_in})"
                )
                continue
            ...
            if combined_scores is not None:
                ...
                print(
                    f"  {name}: n={n_in}  (good={n_good_in}  bad={n_bad_in})  AUROC={auroc_in:.4f}"
                )
            else:
                print(
                    f"  {name}: n={n_in}  (good={n_good_in}  bad={n_bad_in})  AUROC=n/a"
                )
            strat[name] = entry
...
def aggregate(
    ...
    def _cat_lengths(key: str) -> NDArray | None:
        if chunks[0].get(key) is None:
            return None
        return np.array([length for c in chunks for length in c[key]], dtype=np.int32)
    ...
    print(f"  layers: {layers}")
    print(
        f"  probe_lr={probe_lr}  probe_epochs={probe_epochs}  "
