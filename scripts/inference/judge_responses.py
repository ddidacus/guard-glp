...
def run(
    ...
    evaluator_llm: Any = AutoModelForCausalLM.from_pretrained(
        evaluator_model_id, torch_dtype=torch.float16
    )
    evaluator_llm = evaluator_llm.to(device)
    evaluator_llm.eval()
    ...
    for alpha in alpha_list:
        ...
        with open(responses_file, "w") as f:
            json.dump({"benign": ben_records}, f, indent=2)
