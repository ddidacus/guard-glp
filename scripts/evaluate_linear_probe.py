...
def aggregate(
    ...
    train_good_acts /= torch.norm(train_good_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    train_bad_acts /= torch.norm(train_bad_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    cal_good_acts /= torch.norm(cal_good_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    cal_bad_acts /= torch.norm(cal_bad_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    test_good_acts /= torch.norm(test_good_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    test_bad_acts /= torch.norm(test_bad_acts, p=2, dim=-1).unsqueeze(-1) + 1e-20
    print("done")
...
if __name__ == "__main__":
    ...
    def run(config: str = "eval_config.yaml", gpu_id: int = 0):
        ...
        out_file = out_dir / f"acts_{gpu_id}.th"
        if out_file.exists():
            print(f"[GPU {gpu_id}] Activations already exist at {out_file}, skipping extraction.")
            return
        if gpu_id == 0:
