...
def extract_activations(
    ...
    with torch.no_grad():
        ...
        reconstructed_layers: list[torch.Tensor] = []
        noisy_denorm_layers: list[torch.Tensor] = []
        ...
        for li, actual_layer in enumerate(layers):
            ...
            reconstructed_layers.append(
                diffusion_model.normalizer.denormalize(
                    reconstructed, layer_idx=actual_layer
                )
            )
            noisy_denorm_layers.append(
                diffusion_model.normalizer.denormalize(noisy, layer_idx=actual_layer)
            )

        reconstructed_acts = torch.cat(reconstructed_layers, dim=1)  # (N, L, D)
        noisy_acts = torch.cat(noisy_denorm_layers, dim=1)  # (N, L, D)
        reconstructed_acts = reconstructed_acts.to(
        ...
        errors = torch.norm(
            (activations - reconstructed_acts)
            .float()
            .reshape(activations.shape[0], -1),
            p=2,
            dim=1,
        )
...
def extract_main(
    ...
    llm_model: Any = AutoModelForCausalLM.from_pretrained(
        llm_model_id, torch_dtype=torch.bfloat16, device_map=device
    )
    llm_tokenizer: Any = AutoTokenizer.from_pretrained(llm_model_id)
    diffusion_model: Any = load_glp(glp_model_id, device=device, checkpoint="final")
    ...
    print(
        f"[+] shard: \t\t{gpu_id}/{num_gpus}  ({len(good_set)} good, {len(bad_set)} bad)"
    )
    print("================================================")

    print(f"======= Encoding the good set (GPU {gpu_id}) =======")
    result_good = encode_prompts(
        good_set,
        batch_size,
        llm_model,
        llm_tokenizer,
...
def _pca_fit_transform(
    arrays: list[np.ndarray[Any, Any]], k: int | None = None
) -> tuple[torch.Tensor, list[np.ndarray[Any, Any]]]:
...
def _tsne_fit_transform(
    arrays: list[np.ndarray[Any, Any]], k: int = 2, perplexity: float = 30.0
) -> list[np.ndarray[Any, Any]]:
    sizes = [a.shape[0] for a in arrays]
    combined = np.concatenate(arrays, axis=0)
    embedded = TSNE(
        n_components=k, perplexity=perplexity, random_state=42
...
def plot_pca_distributions_layerwise(
    ...
    for idx, layer_num in enumerate(layer_indices):
        ...
        if method == "pca":
            ...
            recon_good_proj = (
                (
                    torch.from_numpy(recon_good.copy()).float()
                    - torch.from_numpy(recon_good.copy()).float().mean(0, keepdim=True)
                )
                @ W
            ).numpy()
            recon_bad_proj = (
                (
                    torch.from_numpy(recon_bad.copy()).float()
                    - torch.from_numpy(recon_bad.copy()).float().mean(0, keepdim=True)
                )
                @ W
            ).numpy()
            ...
            good_err_proj = (
                (good_err_t - good_err_t.mean(0, keepdim=True)) @ W
            ).numpy()
        ...
        def comp_label(c: int) -> str:
            return f"PC {c + 1}" if method == "pca" else f"t-SNE {c + 1}"

        for data_pair, fname_suffix, title_prefix in [
            (
                (good_proj, bad_proj),
                f"{prefix}_{layer_num}.png",
                f"{method_label} Component Distributions",
            ),
            (
                (recon_good_proj, recon_bad_proj),
                f"{prefix}_reconstructions_{layer_num}.png",
                f"Reconstructed {method_label} Component Distributions",
            ),
            (
                (good_err_proj, bad_err_proj),
                f"{prefix}_errors_{layer_num}.png",
                "Per-Component Reconstruction Error",
            ),
        ]:
            gp, bp = data_pair
            fig, axes_obj = plt.subplots(
                n_comp_actual, 1, figsize=(10, 2.5 * n_comp_actual), sharex=False
            )
            axes: Any = [axes_obj] if n_comp_actual == 1 else axes_obj
...
def plot_error_comparison(
    ...
    fig, axes_obj = plt.subplots(
        len(layer_indices), 1, figsize=(10, 2.5 * len(layer_indices)), sharex=False
    )
    axes: Any = [axes_obj] if len(layer_indices) == 1 else axes_obj
    ...
    for idx, layer_num in enumerate(layer_indices):
        good_err_layer = np.linalg.norm(
            np.abs(good[:, idx, :] - recon_good[:, idx, :]), ord=2, axis=1
        )
        bad_err_layer = np.linalg.norm(
            np.abs(bad[:, idx, :] - recon_bad[:, idx, :]), ord=2, axis=1
        )
...
def plot_error_by_layer(
    error_gap_stats: dict[str, dict[str, float]],
    out_dir: str,
    prefix: str = "error_by_layer",
) -> None:
...
def aggregate_main(
    results_dir: str, layers: str | int | list[int] = "1,7,15", method: str = "pca"
) -> None:
    ...
    error_gap_stats: dict[str, dict[str, float]] = {}
    for idx, layer_num in enumerate(layer_list):
        good_err = np.linalg.norm(
            np.abs(acts_good_np[:, idx, :] - recon_good_np[:, idx, :]), ord=2, axis=1
        )
        bad_err = np.linalg.norm(
            np.abs(acts_bad_np[:, idx, :] - recon_bad_np[:, idx, :]), ord=2, axis=1
        )
        print(
            f"  Layer {layer_num:2d}: good={good_err.mean():.4f} ± {good_err.std():.4f}  "
    ...
    print(
        f"{method.upper()} scatter plots saved to {results_dir}/{method}_scatter2d_layer*.png/.pdf"
    )
